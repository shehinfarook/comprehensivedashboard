  ...
  return (
    ...
    {/* Removed logo and brand name for cleaner look */}
        ...
        <form onSubmit={handleSignup} style={{ width: '100%', maxWidth: 360 }}>
          ...
          <Button type="submit" variant="contained" fullWidth sx={{ bgcolor: '#8e6be6', color: '#fff', fontWeight: 600, py: 1.5, fontSize: 16, borderRadius: 2, boxShadow: 2, mt: 2 }} disabled={!agree}>Sign up</Button>
          {error && <Typography color="error" sx={{ mt: 2 }}>{error}</Typography>}
          {success && <Typography color="primary" sx={{ mt: 2 }}>{success}</Typography>}
        </form>
        <Typography mt={2} color="text.secondary" fontSize={15}>
          Already have an account? <a href="/" style={{ color: '#8e6be6', textDecoration: 'underline' }}>Login</a>
        </Typography>
      </Box>
