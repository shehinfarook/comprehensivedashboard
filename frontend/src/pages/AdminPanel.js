...
function AdminPanel() {
  return (
  <Box minHeight="100vh" display="flex" bgcolor="#f5f5fa" className="admin-panel-container glassy-bg">
      {/* Sidebar */}
      <Paper elevation={3} sx={{ width: 180, bgcolor: '#8e6be6', color: '#fff', pt: 4, borderTopRightRadius: 24, borderBottomRightRadius: 24 }}>
        <List>
          <ListItem>
      ...
      {/* Main Content */}
      <Box flex={1} p={6}>
        <Typography variant="h4" fontWeight={700} mb={4} color="#8e6be6">Admin Panel</Typography>
        <Grid container spacing={4}>
          ...
          <Grid item xs={12} md={12}>
            <Card elevation={6} sx={{ borderRadius: 4, bgcolor: '#e8f0fe', boxShadow: 8, mb: 4, transition: 'box-shadow 0.3s', ':hover': { boxShadow: 16, bgcolor: '#d1e1fc' } }}>
              <CardContent>
                <Typography variant="h6" mb={2} color="#1e88e5">Intern List for Performance Charting</Typography>
                <AdminInternList />
              </CardContent>
            </Card>
          </Grid>
